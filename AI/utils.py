...
class ModelClient:
    ...
    def invoke(self, prompt: str, user_input: str, max_tokens: int = 1024, temperature: float = 0) -> str:
    ...
    def retrieve(self, query_text: str, specific_system_prompt: str="", num_results: int = 10) -> str:
        """
...
def obtener_deuda_total_y_documentos(json_dict):
    ...
    if direct_debt_details:
        for detalle in direct_debt_details:
            deuda = detalle.get('total_credit', '0').replace('$', '').replace('.', '').replace(',', '')
            deuda_total += int(deuda)
            cantidad_documentos += 1
            
            current = detalle.get('current', '0').replace('$', '').replace('.', '').replace(',', '')
            if int(current) > 0:
                detalle_documentos_no_pagados.append({
                    "tipo": "directa",
                    "institucion": detalle.get("financial_institution", "Desconocida"),
    ...
    if indirect_debt_details:
        for detalle in indirect_debt_details:
            deuda_indirecta = detalle.get('total_credit', '0').replace('$', '').replace('.', '').replace(',', '')
