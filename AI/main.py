...
from prompts import (CONSOLIDADOR, PUNTAJERO, RECOMENDADOR, RESUMIDOR,
                     USER_FINANCIAL_INFORMATION, USER_PERFONAL_INFORMATION)
...
def parse_arguments():
    ...
    parser.add_argument(
        "--ingreso_mensual", type=int, required=True, help="Ingreso mensual del usuario"
    )
    parser.add_argument(
        "--enfoque", type=str, required=True, help="Enfoque del análisis"
    )
...
def main():
    ...
    user_prompt = (
        USER_PERFONAL_INFORMATION.format(
            edad=edad, ingreso_mensual=ingreso_mensual, enfoque=enfoque
        )
        + USER_FINANCIAL_INFORMATION.format(**json_strings)
        + "\n<Documentos> \n"
        + retrieved_docs
        + "\n</Documentos>"
    )
    ...
    resumen_general = (
        response.split("<Resumen general>")[1].split("</Resumen general>")[0].strip()
    )
    ...
    categorias = (
        categories_recomendaciones.split("<Categorias>")[1]
        .split("</Categorias>")[0]
        .strip()
    )
    recomendaciones = (
        categories_recomendaciones.split("<Recomendaciones>")[1]
        .split("</Recomendaciones>")[0]
        .strip()
    )
    ...
    razon_puntaje = (
        puntaje_final.split("<Razonamiento>")[1].split("</Razonamiento>")[0].strip()
    )
